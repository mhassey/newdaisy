...
import com.daisyy.broadcast.broadcastforbackgroundservice.AlaramHelperBackground;
import com.daisyy.common.session.SessionManager;
import com.daisyy.pojo.LogsDataPojo;
...
//        Date date = localToGMT();
    ...
    public static void deleteCardFolder() {
        File dir;
        if (android.os.Build.VERSION.SDK_INT > android.os.Build.VERSION_CODES.Q) {
            dir = new File(AppController.getInstance().getExternalFilesDir(""), Constraint.SLASH + Constraint.FOLDER_NAME + Constraint.SLASH + Constraint.CARD);
        } else
